...
class PropertiesService {
  ...
  async createProperty(propertyData, ownerId) {
    // --- Validation ---
console.log('🧪 Service received propertyData:', propertyData)

// 1️⃣ Check that propertyTypeId exists
if (!propertyData.propertyTypeId) {
  const err = new Error('propertyTypeId is required')
  err.status = 400
  throw err
}

// 2️⃣ Trim and validate UUID
propertyData.propertyTypeId = propertyData.propertyTypeId.trim()

if (!isValidUUID(propertyData.propertyTypeId)) {
  const err = new Error('propertyTypeId must be a valid UUID')
  err.status = 400
  throw err
}

// 3️⃣ Check if property type exists in DB
...
if (!propertyType) {
  const err = new Error(
    `propertyTypeId "${propertyData.propertyTypeId}" does not exist`
  )
  err.status = 400
  throw err
}

console.log('✅ propertyTypeId is valid and exists:', propertyData.propertyTypeId)
    ...
    if (!propertyCode) {
      ...
      if (propertyData.propertyTypeId) {
       ...
       `  Invalid propertyTypeId: ${propertyData.propertyTypeId}`
        )}
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      console.log('🧪 FINAL payload propertyTypeId:', propertyData.propertyTypeId)
